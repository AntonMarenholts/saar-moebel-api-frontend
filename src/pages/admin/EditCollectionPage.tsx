...
import ProductService, { type Category, type UpdateCollectionData } from '../../services/product.service';
...
export default function EditCollectionPage() {
    ...
    useEffect(() => {
        ...
        ProductService.getCollectionById(collectionId).then(collection => {
            reset({
            ...
            setUploadedImageUrl(collection.imageUrl);
        });
    }, [collectionId, reset]);
    ...
    const onSubmit: SubmitHandler<FormInputs> = (data) => {
        ...
        ProductService.updateCollection(collectionId, collectionData).then(() => {
            setMessage(t('collection_update_success'));
            setIsError(false);
            setTimeout(() => navigate(-1), 2000);
        }).catch((err) => {
            setMessage(err.response?.data?.message || t('collection_update_error'));
            setIsError(true);
        });
    };
    ...
    return (
        <div className="max-w-2xl mx-auto p-8 bg-white rounded-lg shadow-md">
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-bold text-gray-800">{t('editing_collection_title')}</h1>
                <Link to="/admin/add-collection" className="text-sm text-brand-blue hover:underline">
                    {t('back_to_collections_list')}
                </Link>
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div>
                    ...
                    <input id="slug" {...register('slug', { required: t('field_is_required') })} className="mt-1 w-full p-2 border border-gray-300 rounded-md" />
